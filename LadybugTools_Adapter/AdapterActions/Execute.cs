...
namespace BH.Adapter.LadybugTools
{
    public partial class LadybugToolsAdapter : BHoMAdapter
    {
        ...
        private List<object> RunCommand(UTCIHeatPlotCommand command, ActionConfig actionConfig)
        {
            ...
            // run the process
            string cmdCommand = $"{m_environment.Executable} \"{script}\" -e \"{epwFile}\" -in \"{argFile}\" -ws \"{command.WindSpeedMultiplier}\"{(command.UseImperial? " -ip": "")} -sp \"{command.OutputLocation}\"";
            string result = "";
