...
def test_sun_blocked_south():
    south_shelter = Shelter(radiation_porosity=0, altitude_range=(0, 90), azimuth_range=(90, 270))
    south_suns = [
...
def test_sun_blocked_north():
    north_shelter = Shelter(radiation_porosity=0, altitude_range=(0, 90), azimuth_range=(270, 90))
    north_suns = [
...
def test_sky_blocked_opaque():
    shelter = Shelter(radiation_porosity=0, altitude_range=(0, 90), azimuth_range=(0, 360))
    assert shelter.sky_blocked() == 1


def test_sky_blocked_porous():
    shelter = Shelter(radiation_porosity=0.5, altitude_range=(0, 90), azimuth_range=(0, 360))
