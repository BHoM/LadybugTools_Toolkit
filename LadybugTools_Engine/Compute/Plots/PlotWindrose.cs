...
namespace BH.Engine.LadybugTools
{
    public static partial class Compute
    {
        ...
        public static string PlotWindrose(string epwFile, int monthFrom = 1, int dayFrom = 1, int hourFrom = 0, int monthTo = 12, int dayTo = 31, int hourTo = 23, int noBins = 36, string colourMap = "viridis", string outputLocation = null)
        {
            if (epwFile == null)
            {
            ...
            string script = Path.Combine(Python.Query.DirectoryCode(), "LadybugTools_Toolkit\\src\\ladybugtools_toolkit\\bhom\\wrapped\\plot", "windrose.py");

            // run the process
            string command = $"{env.Executable} {script} -e \"{epwFile}\" -m0 \"{monthFrom}\" -d0 \"{dayFrom}\" -h0 \"{hourFrom}\" -m1 \"{monthTo}\" -d1 \"{dayTo}\" -h1 \"{hourTo}\" -cmap \"{colourMap}\" -bins \"{noBins}\" -p \"{outputLocation}\"";
