...
namespace BH.Engine.LadybugTools
{
    public static partial class Create
    {
        ...
        public static AnalysisPeriod AnalysisPeriod(int stMonth = 1, int stDay = 1, int stHour = 0, int endMonth = 12, int endDay = 31, int endHour = 23, bool isLeapYear = false, int timestep = 1)
        {
            ...
            {
                BH.Engine.Base.Compute.RecordError("stMonth and endMonth must be 1-12");
                return null;
            }

            if (stDay < 1 || stDay > 31 || endDay < 1 || endDay > 31)
            {
                BH.Engine.Base.Compute.RecordError("stDay and endDay must be 1-12");
                return null;
            }

            if (stHour < 0 || stHour > 23 || endHour < 0 || endHour > 23)
            {
                BH.Engine.Base.Compute.RecordError("stHour and endHour must be 1-12");
                return null;
            }
            ...
            {
                BH.Engine.Base.Compute.RecordError("timestep must be one of {1, 2, 3, 4, 5, 6, 10, 12, 15, 20, 30, 60}");
                return null;
            }
