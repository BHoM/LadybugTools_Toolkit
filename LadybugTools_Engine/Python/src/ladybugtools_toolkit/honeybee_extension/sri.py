...
def calculate_sri(
    ...
    if not 0 <= solar_reflectance <= 1:
        raise ValueError("Solar reflectance must be between 0 (black body) - 1 (white-body).")
    if not 0 <= thermal_emittance <= 1:
        raise ValueError("Thermal emittance must be between 0 (white-body) - 1 (black-body).")
    if wind_speed < 0:
    ...
    while not np.isclose(
        ...
        - (
            ...
            * (blackbody_surface_temperature**4 - sky_temperature**4)
            + wind_convection_coefficient * (blackbody_surface_temperature - air_temperature)
        ),
        0,
        atol=threshold,
    ):
    ...
    while not np.isclose(
        ...
        - (
            ...
            * (whitebody_surface_temperature**4 - sky_temperature**4)
            + wind_convection_coefficient * (whitebody_surface_temperature - air_temperature)
        ),
        0,
        atol=threshold,
    ):
...
def estimate_sri_properties(
    ...
    model.fit(data[["solar_absorptivity", "thermal_absorptivity"]].values, data["sri"].values)
    ...
    if sa_ <= 0 or sa_ >= 1:
        CONSOLE_LOGGER.error(
            ("Solar absorptivity estimation is beyond allowable limits " "for the target SRI.")
        )
