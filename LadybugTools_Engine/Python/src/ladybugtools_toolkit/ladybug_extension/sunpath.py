...
def sunrise_sunset_azimuths(sunpath: Sunpath, year: int, month: int, day: int) -> dict:
    ...
    azimuths_altitude = {
        k: {"time": datetime(year, v.month, v.day, v.hour, v.minute), "azimuth": sunpath.calculate_sun_from_date_time(v).azimuth} if k in ["sunrise", "sunset"] else {"time": datetime(year, v.month, v.day, v.hour, v.minute), "altitude": sunpath.calculate_sun_from_date_time(v).altitude} for k, v in sunrise_sunset.items() 
    }
