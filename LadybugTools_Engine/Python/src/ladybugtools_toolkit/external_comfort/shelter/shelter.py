...
from ...ladybug_extension.helpers import cardinality
from .sky_occlusion import sky_occlusion
...
class Shelter:
    ...
    def __init__(
        ...
        self._sky_occlusion = sky_occlusion(
            self._start_altitude_radians,
            self._end_altitude_radians,
            self._width_radians,
            self.porosity,
        )
        self._sky_visible = 1 - self._sky_occlusion
    ...
    def effective_wind_speed(self, epw: EPW) -> HourlyContinuousCollection:
        ...
        edge_acceleration_factor = (
            1.1  # amount by whcih to increase wind speed by for edge effects
        )
