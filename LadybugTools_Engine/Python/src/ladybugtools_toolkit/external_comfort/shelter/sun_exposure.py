...
def sun_exposure_location(shelters: List[Shelter], epw: EPW, sun_hours: HourlyContinuousCollection) -> List[float]:
    """Return NaN if sun below horizon, and a value between 0-1 for sun-hidden to sun-exposed.
    ...
    suns = sun_position_list(epw)
    # change sun is up list to pre calculated sun hours list
    sun_is_up = np.array([x > 0 for x in sun_hours.values])
