...
def diurnal(
    epw_file,
    return_file: str,
    data_type_key="Dry Bulb Temperature",
    color="#000000",
    title=None,
    period="monthly",
    save_path=None,
    ...
    try:
        ...
        from ladybugtools_toolkit.bhom.wrapped.metadata.collection import \
            collection_metadata
        ...
        if data_type_key == "Wet Bulb Temperature":
            ...
            coll = HourlyContinuousCollection.from_dict(
                [
                    a
                    for a in epw.to_dict()["data_collections"]
                    if a["header"]["data_type"]["name"] == data_type_key
                ][0]
            )

        fig = diurnal(
            collection_to_series(coll), title=title, period=period, color=color
        ).get_figure()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=("Given an EPW file path, extract a diurnal plot")
    )
    parser.add_argument(
        "-e",
        "--epw_file",
