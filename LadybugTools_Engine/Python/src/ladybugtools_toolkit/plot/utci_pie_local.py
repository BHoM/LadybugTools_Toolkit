...
from ladybugtools_toolkit.plot.colormaps_local import UTCI_LOCAL_COLORMAP, UTCI_LOCAL_LABELS, UTCI_LOCAL_LEVELS
...
def utci_pie_local(
    ...
    series_cut = pd.cut(series, bins=[-100] + UTCI_LOCAL_LEVELS + [100], labels=UTCI_LOCAL_LABELS)
    sizes = (series_cut.value_counts() / len(series))[UTCI_LOCAL_LABELS]
    colors = (
    ...
    if show_legend:
        ...
        legend_elements = [
            ...
            for label, color in list(zip(*[UTCI_LOCAL_LABELS, colors]))
        ]
