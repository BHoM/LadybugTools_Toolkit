...
def seasonality_comparison(epw: EPW, ax: plt.Axes = None) -> plt.Axes:
    ...
    from_day_length = seasonality_from_day_length(epw=epw).rename(
        "From day-length", inplace=False
    )
    ...
    plt.legend(
        new_handles, d.keys(), bbox_to_anchor=(0.5, -0.12), loc="upper center", ncol=4
    )
