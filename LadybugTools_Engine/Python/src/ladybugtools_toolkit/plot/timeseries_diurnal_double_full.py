...
# @analytics
def timeseries_diurnal_double_full(
    series1: pd.Series,
    series2: pd.Series,
    ...
    _min1 = grp1.min().reindex(target_index)
    _lower1 = grp1.quantile(0.05).reindex(target_index)
    _mean1 = grp1.mean().reindex(target_index)
    _upper1 = grp1.quantile(0.95).reindex(target_index)
    _max1 = grp1.max().reindex(target_index)

    grp2 = series2.groupby([series2.index.month, series2.index.hour], axis=0)
    _min2 = grp2.min().reindex(target_index)
    _lower2 = grp2.quantile(0.05).reindex(target_index)
    _mean2 = grp2.mean().reindex(target_index)
    _upper2 = grp2.quantile(0.95).reindex(target_index)
    _max2 = grp2.max().reindex(target_index)
    ...
    """
    mlines.Line2D([0], [0], label=s1name + " 5-95%ile", color=color1, lw=1, ls=":"),
    mlines.Line2D([0], [0], label=s2name + " 5-95%ile", color=color1, lw=1, ls=":"),
    mpatches.Patch(color=color1, label=s1name + " Range", alpha=0.3),
    mpatches.Patch(color=color2, label=s2name + " Range", alpha=0.3),
    """
