...
class SpatialComfort:
    ...
    unshaded_typology_result: TypologyResult = field(init=False, repr=False)
    shaded_typology_result: TypologyResult = field(init=False, repr=False)

    utci_simple_possible: bool = field(init=False, repr=False)
    utci_complex_possible: bool = field(init=False, repr=False)
    ...
    def _simulation_validity(self) -> None:
        ...
        object.__setattr__(self, "utci_simple_possible", True)

        water_sources_json = self.simulation_directory / "water_sources.json"
        if not (water_sources_json).exists():
            object.__setattr__(self, "utci_complex_possible", False)
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        d = {
            ...
            "shaded_ground_temperature": self.shaded_typology_result.external_comfort_result.shaded_below_temperature,
            "shaded_universal_thermal_climate_index": self.shaded_typology_result.universal_thermal_climate_index,
            "shaded_mean_radiant_temperature": self.shaded_typology_result.mean_radiant_temperature,
            "unshaded_ground_temperature": self.unshaded_typology_result.external_comfort_result.unshaded_below_temperature,
            "unshaded_universal_thermal_climate_index": self.unshaded_typology_result.universal_thermal_climate_index,
            "unshaded_mean_radiant_temperature": self.unshaded_typology_result.mean_radiant_temperature,
        }
        return d
    ...
    def _typology_result(self) -> Dict[str, TypologyResult]:
        ...
        return {"unshaded_typology_result": unshaded, "shaded_typology_result": shaded}
...
class SpatialComfortResult:
    ...
    def __post_init__(self) -> SpatialComfortResult:
        ...
        # Calculate spatial MRT matrix
        object.__setattr__(
            self, "mean_radiant_temperature", self._mean_radiant_temperature()
        )

        # Create triangulation objects necessary for plotting spatial values
...
#         self._generic_output()
    ...
    def _points(self) -> pd.DataFrame:
        ...
        try:
            ...
            if points_path.exists():
                print(f"{self.spatial_comfort.simulation_directory.name} - Loading points data")
                return pd.read_hdf(points_path, "df")
            else:
                print(f"{self.spatial_comfort.simulation_directory.name} - Processing points data")
                points_files = list(
                    (
                ...
                points = load_pts(points_files).drop(columns=['vx', 'vy', 'vz'], axis=1, level=1).astype(np.float16)
    ...
    def _total_irradiance(self) -> pd.DataFrame:
        ...
        try:
            ...
            if total_irradiance_path.exists():
                print(f"{self.spatial_comfort.simulation_directory.name} - Loading irradiance data")
                return pd.read_hdf(total_irradiance_path, "df")
            else:
                print(f"{self.spatial_comfort.simulation_directory.name} - Processing irradiance data")
                ill_files = list(
                    (
                        self.spatial_comfort.simulation_directory
                        / "annual_irradiance"
                ...
                total_irradiance = make_annual(load_ill(ill_files)).fillna(0).astype(np.float16).clip(lower=0)
    ...
    def _sky_view(self) -> pd.DataFrame:
        ...
        try:
            ...
            if sky_view_path.exists():
                print(f"{self.spatial_comfort.simulation_directory.name} - Loading sky-view data")
                return pd.read_hdf(sky_view_path, "df")
            else:
                print(f"{self.spatial_comfort.simulation_directory.name} - Processing sky-view data")
                res_files = list(
                    (
                        self.spatial_comfort.simulation_directory
                        / "sky_view"
    ...
    # METHODS AND PROPERTIES BELOW HERE FOR MOISTURE PROXY SIMULATION
    
    @property
    def points_xyz(self) -> pd.DataFrame:
        reshaped_points = self.points.unstack().reset_index().pivot(index=["level_0", "level_2"], values=0, columns="level_1").dropna()
        reshaped_points.columns.name = None
        reshaped_points.index.names = (None, None)
        return reshaped_points

    @property
    def moisture_sources(self) -> List[MoistureSource]:
        ...
        moisture_sources_path = (
            self.spatial_comfort.simulation_directory / "moisture_sources.json"
        )

        if not moisture_sources_path.exists():
            raise FileNotFoundError("Moisture sources file not found")
    ...
    def moisture_source_points(self) -> List[List[List[float]]]:
        return [np.array(i.points)[:, :2] for i in self.moisture_sources]
    
    @property
    def moisture_source_magnitudes(self) -> List[float]:
        return [i.magnitude for i in self.moisture_sources]
    
    @cached_property
    def moisture_source_vectors(self) -> List[List[List[List[float]]]]:
        return [np.subtract(i, self.points_xyz.values[:, :2][:, None]) for i in self.moisture_source_points]
    ...
    def moisture_source_angles(self) -> List[List[List[float]]]:
        return [self.angle_from_north(i.T).T for i in self.moisture_source_vectors]
...
#         )
    ...
    def _mean_radiant_temperature(self) -> pd.DataFrame:
        ...
        try:
            ...
            if mrt_path.exists():
                print(
                    f"{self.spatial_comfort.simulation_directory.name} - Loading mean-radiant-temperature data"
                )
                return pd.read_hdf(mrt_path, "df")

            print(f"{self.spatial_comfort.simulation_directory.name} - Processing mean-radiant-temperature data")
...
#                 self.spatial_comfort.shaded_typology_result.ground_surface_temperature,
#                 self.spatial_comfort.unshaded_typology_result.ground_surface_temperature,
#                 self.total_irradiance,
#                 self.sky_view,
#                 to_array(
#                     self.spatial_comfort.external_comfort_result.external_comfort.epw.global_horizontal_radiation
...
#         # TODO - MAKE THE HEADER INDEX THE SAME AS THE SIMPLE HEADER INDEX TO GET THE DOWNSTREAM STUFF WORKING!!!!!
#         try:
