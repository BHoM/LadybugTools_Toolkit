...
class ShelterNew:
    def __init__(self, porosity: float = 0.0, altitude_range: Tuple[float] = (0, 90), azimuth_range: Tuple[float] = (0, 360)):
    ...
    def __eq__(self, __o: object) -> bool:
        if (self.altitude_range == __o.altitude_range) and (self.azimuth_range == __o.azimuth_range):
            return True
        return False
    ...
    def sky_occluded(self) -> float:
        ...
        return (np.sin(self.end_altitude_radians) - np.sin(self.start_altitude_radians)) * self.width_radians / (2 * np.pi)
    ...
    def sun_blocked(self, suns: Union[List[Sun], Sun]) -> List[bool]:
        ...
        for s in suns:
            ...
            if self.crosses_north:
                in_azimuth_range = (s.azimuth > self.start_azimuth) or (s.azimuth < self.end_azimuth)
            else:
...
def coincident_shelters(shelters: List[ShelterNew]) -> List[ShelterNew]:
    """Return True if shelters cover the same area of sky"""
